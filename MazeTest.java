package Maze;

import java.awt.BorderLayout;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import javax.swing.JOptionPane;
import java.io.BufferedReader;
import java.io.FileReader;
import java.util.ArrayList;
import javax.swing.JButton;
import javax.swing.JFrame;
import javax.swing.JTextArea;
import javax.swing.JPanel;

/**
 * A class to test Maze.java.
 * @author Koffman and Wolfgang
 */
public class MazeTest extends JFrame implements GridColors {

    /**
     *
     */
    private static final long serialVersionUID = 1L;
    // data field
    private final TwoDimGrid theGrid; // a 2-D grid of buttons

    /** Reads data file and defines array bitMap to match data file */
    public static void main(final String[] args) {
        try {
            if (args.length < 1) {
                // no file name given
                String reply = JOptionPane.showInputDialog("Enter number of rows");
                final int nRows = Integer.parseInt(reply);
                reply = JOptionPane.showInputDialog("Enter number of columns");
                final int nCols = Integer.parseInt(reply);
                final TwoDimGrid aGrid = new TwoDimGrid(nRows, nCols);
                new MazeTest(aGrid);
            } else {
                // Create array bitMap from a data file
                final BufferedReader br = new BufferedReader(new FileReader(args[0]));

                // Read each data line (a string) into
                // gridArrayList. Each element is a char array.
                final ArrayList<char[]> gridArrayList = new ArrayList<char[]>();
                String line;
                while ((line = br.readLine()) != null) {
                    final char[] row = line.toCharArray();
                    gridArrayList.add(row);
                }

                // bitMap is a 2-D array based on data in gridArrayList
                final char[][] bitMap = gridArrayList.toArray(new char[gridArrayList.size()][]);
                final int nRows = bitMap.length;
                final int nCols = bitMap[0].length;

                // create a new TwoDimGrid and recolor it based on bitMap
                final TwoDimGrid aGrid = new TwoDimGrid(nRows, nCols);
                aGrid.recolor(bitMap, NON_BACKGROUND);
                new MazeTest(aGrid);
            }
        } catch (final Exception ex) {
            System.err.println("Exception " + ex);
            ex.printStackTrace();
            System.exit(1);
        }
    }

    // Builds the GUI
    private MazeTest(final TwoDimGrid aGrid) {
        theGrid = aGrid;
        getContentPane().add(aGrid, BorderLayout.CENTER);
        // Blob aBlob = new Blob(aGrid);
        final JTextArea instruct = new JTextArea(2, 20);
        instruct.setText("Toggle a button to change its color" + "\nPress SOLVE when ready");
        getContentPane().add(instruct, BorderLayout.NORTH);
        final JButton solveButton = new JButton("SOLVE");
        solveButton.addActionListener(new ActionListener() {

            public void actionPerformed(final ActionEvent e) {
                solve();
            }
        });
        final JButton resetButton = new JButton("RESET");
        resetButton.addActionListener(new ActionListener() {

            public void actionPerformed(final ActionEvent e) {
                (new Maze(theGrid)).restore();
            }

        });
        final JPanel bottomPanel = new JPanel();
        bottomPanel.add(solveButton);
        bottomPanel.add(resetButton);
        getContentPane().add(bottomPanel, BorderLayout.SOUTH);
        setDefaultCloseOperation(DISPOSE_ON_CLOSE);
        pack();
        setVisible(true);
    }

    public void solve() {
        /* Create a new Maze */
        final Maze m = new Maze(theGrid);

        /* Test for PROBLEM2 */
        final ArrayList<ArrayList<PairInt>> result = m.findAllMazePaths(0, 0);// Start from (0,0)
        /* If there is one path at least */
        if (!result.isEmpty()) {
            /* Print all of the paths */
            System.out.println("All the paths:");
            System.out.print("[");
            for (int i = 0; i < result.size(); i++) {
                System.out.print("[");
                for (int j = 0; j < result.get(i).size(); j++) {
                    System.out.print(result.get(i).get(j).toString());
                }
                if (i == result.size() - 1)
                    System.out.println("]]");
                else
                    System.out.println("],");
            }

            /* Test for PROBLEM2 */
            /* Print one of the shortest path */
            System.out.println("The shortest path: ");
            final ArrayList<PairInt> min = m.findMazePathMin(0, 0);
            System.out.print("[");
            for (int i = 0; i < min.size(); i++) {
                System.out.print(min.get(i).toString());
            }
            System.out.println("]");
        }

        /* Test for PROBLEM1 */
        final boolean found = m.findMazePath();
        
        if (found) {
            JOptionPane.showMessageDialog(null, "Success - reset maze and try again");
        } else {
            JOptionPane.showMessageDialog(null, "No path - reset maze and try again");
        }
       
    }
}
